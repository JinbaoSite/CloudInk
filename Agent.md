...
- 登录标识同时支持用户名和邮箱，比较前统一转为小写；注册仍必须分别校验唯一邮箱和符合规则的唯一用户名，并兼容旧客户端提交的 `email` 登录字段。
- 新注册用户必须以 `approved=0`、`approval_status=pending` 保存且不得签发 Cookie 或提前创建工作区；Root 通过侧边栏 Messages 批准或拒绝，批准后设置 `approved=1` 并创建用户工作区。批准和拒绝都必须保留审批状态与处理时间，不得删除审批历史；数据库迁移前的现有用户默认保持已批准且不计入审批历史。
